...
| `MenuFromTemplate`                                   | component     | `AccountSwitcher`                      | Taut's AccountSwitcher insertion point; also discovered by Slick's component-name scan                                                                                                                                                                                                                 |
| `SvgIcon`                                            | component     | `AccountSwitcher`                      | Taut's account-row icons; resolved by Slick's `elements` API from the component display name                                                                                                                                                                                                           |
| `members`                                            | redux slice   | `AccountSwitcher`                      | account rows resolve saved user IDs through `members.useMember`                                                                                                                                                                                                                                        |
| `createSelectorGetMemberById`                        | selector key  | `AccountSwitcher`                      | `members.useMember` finds the selector by its export `meta.key`; handles aliases and shared-channel members                                                                                                                                                                                            |
| `ensureMembersArePresent`                            | thunk         | `AccountSwitcher`                      | `members.useMember` dispatches it when a saved account's profile is not cached                                                                                                                                                                                                                         |
...
| `copyDocumentStylesheets`                            | named export  | core (`src/app/slack/childWindows.ts`) | named function export Slack runs once per pop-out; found by scanning `allExports()` for the name. It is the only moment a pop-out document is settled but not yet rendered into. A rename falls back to the `window.open` backstop in the same file, so styles arrive slightly later rather than never |
